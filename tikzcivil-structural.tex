...
%Defines an axis with two perpendicular directions and one rotation. This is
%intended to be used in the description of degrees of freedom.
%------------------------------------------------------------------------------%
%
\pgfkeys{
  ...
  position/.estore in = \axisPosition,
  angle/.estore in = \axisAngle,
  size/.estore in = \axisSize,
}
...
\pgfkeys{
  ...
  position/.estore in = \framePosition,%position of the left support
  displacement/.estore in = \frameDisplacement,%horizontal displacement at the top
  left support/.estore in = \leftSupport,
  right support/.estore in = \rightSupport,
  dof1/.estore in = \dofOne,
  dof2/.estore in = \dofTwo,
  dof3/.estore in = \dofThree,
  dof4/.estore in = \dofFour,
  dof5/.estore in = \dofFive,
  dof6/.estore in = \dofSix,
}
%
...
\newcommand{\FrameSimple}[1][]{%
  ...
  \coordinate (p1) at (\framePosition) {};
  \coordinate (p2) at ([shift={(\dofOne,\frameHeight+\dofTwo)}]p1) {};
  \coordinate (p3) at ([shift={(\frameWidth+\dofFour,\frameHeight+\dofFive)}]p1) {};
  \coordinate (p4) at ([shift={(\frameWidth,0)}]p1) {};
  ...
  %Frame element 1
  \draw[frame element] (p1) to[out=90,in=-90] (p11) to[out=90,in=-90+\dofThree]
                       (p21) to[out=90+\dofThree,in=-90+\dofThree] (p2);
  %Frame element 2
  \draw[frame element] (p2) to[out=0+\dofThree,in=180+\dofThree] (p22) to[out=0+\dofThree,in=180+\dofSix]
                       (p32) to[out=0+\dofSix,in=180+\dofSix] (p3);
  %Frame element 3
  \draw[frame element] (p3) to[out=-90+\dofSix,in=90+\dofSix] (p33) to[out=-90+\dofSix,in=90]
                       (p43) to[out=-90,in=90] (p4);
  %Supports are drawn
  \ifsupport
    \Support[position = p1];
