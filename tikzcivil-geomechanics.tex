...
\newcommand\RetainingWall[1][]{%
  ...
  \ifgroundleft
    ...
    \coordinate (lg2) at ([shift={(\wallRatio*\baseWidth, \leftGroundH-\baseThickness*0.4)}]w1);
    \coordinate (lg3) at ([shift={(\wallRatio*\baseWidth, \leftGroundH)}]w1);
    \draw[line width=1pt] (lg1) -- (lg3);
    \filldraw[soil style] (lg1) rectangle (lg2);
  \fi
  %base coords
  ...
  \ifgroundright
    ...
    \draw[line width=1pt] (b4) -- (rg1);
    \filldraw[soil style] (b4) rectangle (rg2);
    \pgftransformrotate{-\beta}
  \fi
} 
